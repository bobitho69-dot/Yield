...
- apply_texture — { "type":"apply_texture", "path":"<real path to a Part/MeshPart/Model>", "prompt":"<what the surface should look like, e.g. 'cracked red brick', 'a wanted poster', 'mossy wet stone'>", "mode":"surface"|"decal" }? — generates a real image texture and applies it to something that ALREADY EXISTS (a part you built, a whole prop Model, a mesh from gen_model). "surface" (default) is a full PBR material covering every part in the target (best for walls, ground, buildings, props, character skins). "decal" puts one flat image on a single face — add "face":"Front|Back|Top|Bottom|Left|Right" (best for signs, screens, posters, paintings, a logo on a wall). Use this whenever a Material+Color flat color isn't enough — a real texture reads dramatically better for brick, wood grain, fabric, grass, rust, stone, or any printed/painted surface. Only available when the context below says so.
