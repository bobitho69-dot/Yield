...
| `worker` | `{ name, kind, status, detail?, model? }` | an agent's lifecycle: `status` = `start`/`done`/`fail` (`kind` = build/verify), `model` = the AI it's running on — drives the "who's working" roster |
